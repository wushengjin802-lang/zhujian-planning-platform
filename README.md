...
## v2.0 项目中心起始版本（2026-07-06）

从本版本开始，开发重点从工作台切换到 **项目中心页面**。v2.0 严格限定在 PDD 5.2 项目中心功能边界内，不扩展报告编制、GIS、投资测算、方案比选或工作台新能力。

本轮新增能力：

- 项目中心聚合 API：`GET /api/project-center`；
- 项目详情 API：`GET /api/projects/{project_id}` 返回项目概况、成员、里程碑、初始化状态和可执行动作；
- 新建项目向导：基本信息、模板与密级、成员与默认里程碑；
- 项目成员维护：添加/更新、移除项目成员；
- 项目里程碑维护：新增、更新状态、负责人和计划日期；
- 项目模板版本、密级、计划时间和说明维护；
- 项目状态流转：建档中、进行中、已关闭、已归档；
- 项目复制：可选择复制成员、里程碑、模板与密级；
- 项目编号自动生成与唯一索引；
- 项目中心页面升级为项目台账 + 详情抽屉 + 建档向导。
...
```text
docs/00_project/住建平台功能完成度Checklist_v2.0_20260706.xlsx
```

详见：`docs/00_project/项目中心功能v2.0起始版本说明_20260706.md`。

## v2.1 项目中心初始化包与状态门禁

本版本继续限定在 PDD 5.2 项目中心边界内，重点补齐项目初始化与关闭/归档前置门禁：

- 新增项目初始化包：资料清单、事实框架、章节目录、成果项计划、初始化记录；
- 新建/复制项目时自动补齐初始化包，也支持在项目详情中手动补齐；
- 新增资料清单状态维护，支持“待上传、已上传、已确认、不适用”；
- 新增关闭/归档状态门禁，阻断初始化缺失、严重/阻断问题、运行任务、未审核章节、未生成成果等风险；
- 项目详情增加“初始化包与门禁”页签，展示初始化检查项、资料清单、关闭门禁、归档门禁和初始化记录；
- 新增 API：`POST /api/projects/{project_id}/initialize`、`PATCH /api/projects/{project_id}/materials/{material_id}`、`GET /api/projects/{project_id}/status-gate`；
- 版本号更新为 `2.1.0`，Checklist 更新为 `住建平台功能完成度Checklist_v2.1_20260706.xlsx`。




### v2.2 项目中心继续完善（2026-07-06）

- 新增建档向导草稿：保存、载入、删除，创建项目后自动标记已使用。
- 新增地区/初始化规则：内置通用规则与政府投资项目规则，可作为项目初始化包的资料清单、事实框架、章节目录、成果项来源。
- 项目创建、复制和手动初始化时绑定地区规则版本，初始化记录保存规则ID、名称、版本和预期项数量。
- 项目详情与新建向导支持地区、地区/初始化规则选择，项目中心展示草稿和可用规则。

#
## v2.4 项目中心增强（2026-07-06）

本版本继续限定在 PDD 5.2 项目中心范围内，补齐 v2.3 后遗留的迁移审批、迁移回滚与关闭后只读边界：

- 迁移计划创建后进入“待审批”状态；
- 新增迁移计划审批通过、驳回接口；
- 应用迁移计划前必须完成审批；
- 应用迁移时记录迁移前模板/地区规则指针；
- 新增迁移计划回滚接口，回滚只恢复模板/地区规则指针，不删除迁移期间补齐的资料、事实、章节和成果项；
- 项目关闭/归档后，项目中心基础资料进入只读边界；
- 项目详情返回 readonly、readonlyReason 及迁移计划行级 actions；
- 前端迁移计划增加审批、驳回、应用、回滚操作；
- Checklist 已更新为 `docs/00_project/住建平台功能完成度Checklist_v2.4_20260706.xlsx`。

## v2.3 项目中心迁移差异与修订链（2026-07-06）

本版本继续限定在 PDD 5.2 项目中心边界内，重点补齐 v2.2 后保留的“模板/地区规则迁移差异分析 + 项目修订链基础模型”：

- 新增模板/地区规则迁移差异预览，比较当前项目骨架与目标初始化规则的资料清单、事实框架、章节目录和成果项差异；
- 新增迁移计划表，可保存迁移差异、风险等级、目标模板和目标地区规则版本；
- 新增迁移计划应用动作，应用时仅更新项目模板/地区规则并补齐新增初始化项，不自动删除现有资料、事实、章节或成果；
- 新增项目修订链基础模型，支持创建修订、记录父修订、修订号、原因、状态和基线快照；
- 项目详情新增“迁移与修订”页签，用于查看差异、生成/应用迁移计划、创建/确认修订；
- 版本号更新为 `2.3.0`，Checklist 更新为 `住建平台功能完成度Checklist_v2.3_20260706.xlsx`。

详见：`docs/00_project/项目中心功能v2.3迁移差异与修订链说明_20260706.md`。

## v2.5 项目中心紧凑化与回滚影响提示（2026-07-06）

本版本继续限定在 PDD 5.2 项目中心范围内，按页面参考图对项目中心顶部区域进行紧凑化调整，同时补充迁移计划回滚影响提示：

- 顶部新增 7 阶段横向流程条，当前项目中心突出第 1 阶段“项目建档｜范围与权限”；
- 项目概览调整为紧凑卡片样式，展示项目总数、进行中、待初始化、高风险、门禁阻断和已归档；
- 迁移计划返回 `rollbackImpact`，说明回滚只恢复模板/地区规则指针，不删除迁移期间补齐的资料、事实、章节或成果项；
- 迁移计划表增加“回滚影响”列；
- 修复前端审批迁移计划重复调用接口的问题；
- 版本号更新为 `2.5.0`。
...
```text
docs/00_project/住建平台功能完成度Checklist_v2.5_20260706.xlsx
```

详见：`docs/00_project/项目中心功能增强说明_v2.5_20260706.md`。
