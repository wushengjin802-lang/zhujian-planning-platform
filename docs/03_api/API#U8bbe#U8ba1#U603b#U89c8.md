...
需携带登录后的 Bearer Token。

查询参数：

- `projectId`：可选。传入时返回当前项目视角；省略时返回全部项目视角。

返回内容：

- `metrics`：项目进度、待办、审核、问题、任务和成果指标；
- `workflow`：资料、事实、章节、质量和成果流程进度；
- `workItems`：根据现有业务状态实时派生的待办；
- `reviewQueue`：章节和投资测算待审核/确认事项；
- `tasks`：资料解析、质量检查和成果导出任务；
- `notifications`：严重问题、待审核、失败任务和未解析资料提醒；
- `recentActivities`：近期审计活动；
- `projects`：项目组合概览。
