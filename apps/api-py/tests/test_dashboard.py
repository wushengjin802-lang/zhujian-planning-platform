...
class DashboardAggregationTest(unittest.TestCase):
    def test_build_dashboard_aggregates_work_and_risk(self):
        ...
        self.assertIn("countersign", first_review_task)
        self.assertIn("gatePassed", first_review_task["countersign"])
        ...
        self.assertTrue(result["capabilities"]["slaNotificationMaterialised"])
        self.assertTrue(result["capabilities"]["taskHeartbeat"])
        self.assertTrue(result["capabilities"]["cardHealth"])
        self.assertTrue(result["capabilities"]["slaAutoEscalation"])
        self.assertTrue(result["capabilities"]["reviewCountersignGate"])
        self.assertTrue(result["capabilities"]["notificationCenter"])
        self.assertTrue(result["capabilities"]["forceTaskTerminate"])
        self.assertTrue(result["capabilities"]["cardFaultTolerance"])
        self.assertIn("cardErrors", result)
        self.assertIn("slaSummary", result)
        self.assertIn("cardHealth", result)
        self.assertGreaterEqual(len(result["cardHealth"]), 4)
        self.assertIn("slaLabel", first_work_item)
        self.assertIn("dueHoursRemaining", first_work_item)
        self.assertIn("actionReasons", first_work_item)
        self.assertIn("heartbeat", result["tasks"][0])
